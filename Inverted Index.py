...
rdd7 = rdd6.reduceByKey(lambda a,b: (a,b) ,numPartitions=1)
rdd8 = rdd7.cogroup(filtered_data, numPartitions=1).filter(lambda x: x[1][0] and x[1][1]).map(lambda x: (x[0], (list(x[1][0]), list(x[1][1]))))


rdd8.saveAsTextFile("/bigd37/assignment2_q2")
